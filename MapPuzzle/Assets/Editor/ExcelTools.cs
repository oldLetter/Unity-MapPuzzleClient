...
public class ExcelTools : EditorWindow
{
	...
	/// 输出格式
	/// </summary>
	private static string[] formatOption=new string[]{ "XML","JSON","CSV"};
	...
	private static void Convert()
	{
		...
		{
            foreach(string assetName in nameList)
            {
                //获取Excel文件的绝对路径
                string excelPath = pathRoot + "/" + assetsPath;
                string outpath = "./Assets/Resources/XMLData/" + assetName;
                //构造Excel工具类
                ...
                if (indexOfFormat == 0)
                {
                    //output=outpath.Replace(".xlsx",".xml");
                    outpath += ".xml";
                    Debug.Log(outpath);
                    excel.ConvertToXml(outpath);
                }
